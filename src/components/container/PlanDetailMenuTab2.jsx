...
function PlanDetailMenuTab2({
  ...
  return (
    <BottomTab closeTab={() => setTabState(null)}>
      <div
        className="absolute w-full h-full top-0 left-0 opacity-70 bg-[#E5E5E5] z-10"
      />
      <section className="
        ...
        {isCreator
          ? (
            <>
              {/* <button type="button" onClick={openInviteModal} className={buttonDefaultClass}>
                <img src={iconSet.plan.linkShareIcon} alt="링크로 초대" className={iconDefaultClass} />
                <span>링크로 초대하기</span>
              </button> */}
              <button type="button" onClick={goToPlanEdit} className={buttonDefaultClass}>
                <img src={iconSet.plan.editPencilIcon} alt="계획 수정" className={iconDefaultClass} />
                <span>기본 정보 수정하기</span>

              </button>
              <button type="button" onClick={deletePlan} className={`${buttonDefaultClass} text-[#F34A68] `}>
                <img src={iconSet.plan.deleteRedIcon} alt="계획 삭제" className={iconDefaultClass} />
                <span>삭제하기</span>
          ...
          : (
            <button type="button" onClick={goOutFromPlan} className={`${buttonDefaultClass} text-[#F34A68] `}>
              <img src={iconSet.plan.deleteRedIcon} alt="나가기" className={iconDefaultClass} />
              <span>나가기</span>
            </button>
          )}
      </section>
